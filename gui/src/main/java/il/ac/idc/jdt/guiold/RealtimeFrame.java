package il.ac.idc.jdt.guiold;

import il.ac.idc.jdt.Triangle;
import il.ac.idc.jdt.guiold.readltime.DatStreamer;
import il.ac.idc.jdt.guiold.readltime.FrameHistorian;
import il.ac.idc.jdt.guiold.readltime.RealtimeTriangulatorThread;
import il.ac.idc.jdt.guiold.readltime.TerrainStreamer;
...
public class RealtimeFrame extends JFrame implements ActionListener, Runnable, ItemListener {
	...
	/**
	 * The streamer which will stream frames to the RealtimeTriangulatorThread.
	 */
	...
	public void paint(Graphics g) {
		...
		Vector<Triangle> nextTriangulatedFrame = null;
		...
		try {
			...
			if (m_currentState == FrameState.STARTED || m_currentState == FrameState.GO_TO_FRAME) {
				if (m_currentState == FrameState.STARTED) {
					...
					nextOriginalFrame = FrameHistorian.getOriginalFrame(m_temporaryFolderPath, m_nextFrame);
				}
				...
				if (m_shouldSimplify == true) {
					if (m_currentState == FrameState.STARTED) {
						...
						// of m_rtt, as it the .pgm is needed to be read again,
						// because m_rtt deletes the frame history
						nextSimplifiedFrame = FrameHistorian.getSimplifiedFrame(m_temporaryFolderPath, m_nextFrame);
					}
				...
				if (m_shouldTriangulate == true) {
					if (m_currentState == FrameState.STARTED) {
						...
						// of m_rtt, as it the .pgm is needed to be read again,
						// because m_rtt deletes the frame history
						nextTriangulatedFrame = FrameHistorian.getTriangulatedFrame(m_temporaryFolderPath, m_nextFrame);
					}
				...
				if (nextTriangulatedFrame != null) {
					...
					TriangulationDrawer td = new TriangulationDrawer(m_pnlTriangulatedFrame.getWidth(), m_pnlTriangulatedFrame.getHeight());
					td.drawTriangulation(m_pnlTriangulatedFrame.getGraphics(), nextTriangulatedFrame);
			...
			System.out.println("An error has occurred while updating screen: " + e.getMessage());
			System.exit(0);
		}
	}
	...
	public void resume() {
		try {
			...
			if (m_currentState == FrameState.PAUSED) {
				...
				if (m_rtt == null) {
					m_rtt = new RealtimeTriangulatorThread(m_streamer, m_terraPath, m_temporaryFolderPath, m_shouldSimplify,
							m_shouldTriangulate);
	...
	public void actionPerformed(ActionEvent e) {
		try {
			...
			if (command.compareTo("Start") == 0) {
				...
				if (m_isFirstTimeRun == true) {
					JOptionPane.showMessageDialog(null, "You first need to start" + " the simplification/triangulation process");
				} else {
					String frameNumber = JOptionPane.showInputDialog(null, "Current frame number: " + m_nextFrame.toString()
							+ "\nEnter the frame number: ", null, 1);
			...
			System.out.println("An exception has occurred preforming action: " + ex.getMessage());
		}
