...
public class CounterLine {
	 ...
	 * @see Iterator
	 */
	public Iterator<Point> getPointsListIterator(){
		return new PointIterator(_points.listIterator());
	}
	/**
	 * 
	...
	private class PointIterator implements Iterator<Point> {
		...
		public PointIterator(Iterator<Point_tp> pointsTPItr){
		...
		public Point next() {
			...
			return new Point(tpPoint.getX().doubleValue(),tpPoint.getY().doubleValue(),tpPoint.getZ());
		}
